...
async def get_kitchen_orders(db = Depends(get_db)):
    """
    Get kitchen orders - OPTIMIZED with projection and limit
    """
    # 🔥 PROJECTION: AMBIL FIELD PENTING AJA (CEPAT!)
    ...
    ).sort({"createdAt": -1}).limit(50).to_list(length=50)  # 🔥 LIMIT 50!
...
async def update_kitchen_order_status(order_id: str, status_data: dict, db = Depends(get_db)):
    ...
    if new_status == "cooking" and order.get("status") != "cooking":
